...
function CompanySlider({ cards, slideLeft }: any) {
  ...
  return (
    <div
      className={`bg-[#2E2E2E] w-[100%] h-[106px] rounded-[16px] flex row justify-start items-center gap-[40px]`}
    >
      <motion.div
        id="company-logos"
        initial={{ x: 0 }}
        ...
        className={`flex justify-start gap-[96px] absolute ${slideLeft ? "" : "right-0"}`}
      >
        {cards.map((card: any, i: number) => (
          ...
          >
            <div className="flex items-center h-[90px] md:h-[110px] w-full">
              <Image src={card.image} alt="Company Logo" height={42} className="!max-w-none" />
            </div>
          </div>
        ))}
      </motion.div>
